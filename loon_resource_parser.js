...
var importFilterDrop = 0;
var importMerged = false;
var processStartedAt = Date.now();
var processResultMeta = { nodeCount: 0, subCount: 0, reason: "", excluded: 0, filtered: 0 };
...
function describeConfig() {
  ...
  if (filter) parts.push("筛选节点=" + filter);
...
function parseFilter() {
  ...
  hasFilter = false;
  if (!filter) return;
  var items = splitEscaped(filter, ",");
  for (var i = 0; i < items.length; i++) {
    var item = str(items[i]).trim();
    if (!item) continue;
    var negative = item.charAt(0) === "!";
    var kw = unescapeRenameText(negative ? item.slice(1) : item).trim();
    if (!kw) continue;
    if (negative) filterNegKeywords.push(kw);
    else filterKeywords.push(kw);
  }
  hasFilter = filterKeywords.length > 0 || filterNegKeywords.length > 0;
}
function isFilterKeptName(name) {
  if (!hasFilter) return true;
  var n = str(name);
  for (var i = 0; i < filterNegKeywords.length; i++) {
    if (n.indexOf(filterNegKeywords[i]) !== -1) return false;
  }
  if (!filterKeywords.length) return true;
  for (var j = 0; j < filterKeywords.length; j++) {
    if (n.indexOf(filterKeywords[j]) !== -1) return true;
  }
  return false;
}
function filterItemsByName(items) {
  if (!hasFilter || !items.length) return { kept: items, dropped: 0 };
  var kept = [];
  for (var i = 0; i < items.length; i++) {
    if (isFilterKeptName(items[i].name)) kept.push(items[i]);
  }
  return { kept: kept, dropped: items.length - kept.length };
}
function filterLineItems(items, slots, lines) {
  if (!hasFilter || !items.length) return { kept: items, slots: slots, dropped: 0 };
  var kept = [];
  var keptSlots = [];
  var dropped = 0;
  for (var i = 0; i < items.length; i++) {
    if (isFilterKeptName(items[i].name)) {
      kept.push(items[i]);
  ...
  return { kept: kept, slots: keptSlots, dropped: dropped };
}
function filterKeptLog(keptCount) {
  return hasFilter ? "，筛选保留=" + keptCount : "";
}
...
function processLoonStyle(text) {
  ...
  var loonFilter = filterLineItems(keptItems, keptSlots, lines);
  keptItems = loonFilter.kept;
  keptSlots = loonFilter.slots;
  ...
  processResultMeta.filtered = loonFilter.dropped;
  if (!renamed && !uriRenamed && !uriNormalized && !profileForced && !uriProfileForced && !hasNameMutation() && !hasSort && !hasExcludeConfig() && !hasFilterConfig() && !imports.length) {
  ...
  console.log("[解析器]" + logTag() + "[Loon] 订阅节点=" + items.length + filterKeptLog(keptItems.length));
...
function collectImportItems() {
  ...
  var importFilterResult = filterItemsByName(items);
  importFilterDrop = importFilterResult.dropped;
  items = importFilterResult.kept;
  for (var j = 0; j < items.length; j++) items[j].index = j;
  console.log("[解析器][导入] 导入节点=" + items.length + filterKeptLog(items.length));
...
function processBase64UriList(text) {
  ...
  var base64Filter = filterLineItems(keptNamed, keptSlots, lines);
  keptNamed = base64Filter.kept;
  keptSlots = base64Filter.slots;
  ...
  processResultMeta.excluded = excluded;
  processResultMeta.filtered = base64Filter.dropped;
  if (!renamed && !uriNormalized && !uriProfileForced && !profileForced && !hasNameMutation() && !hasSort && !hasExcludeConfig() && !hasFilterConfig() && !imports.length) {
  ...
  console.log("[解析器]" + logTag() + "[Base64] 订阅节点=" + named.length + filterKeptLog(keptNamed.length));
...
function processClashYaml(text) {
  ...
  processResultMeta.filtered = yamlFilter.dropped;
...
function finish(content, fromUserAgent) {
  logStartupOnce();
  processResultMeta = { nodeCount: 0, subCount: 0, reason: "", excluded: 0, filtered: 0 };
  importExcluded = 0;
  importFilterDrop = 0;
  ...
  var tail = logJoin([
    ...
    { key: "筛选丢弃", value: processResultMeta.filtered + importFilterDrop, zero: true }
  ]);
